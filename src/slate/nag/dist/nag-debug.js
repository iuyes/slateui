 ...
 *
 */
define("slate/nag/1.0.0/nag-debug", [ "$-debug" ], function(require, exports, module) {
    var $ = require("$-debug");
    $.fn.nag = function(parameters) {
        ...
        $(this).each(function() {
            var settings = $.extend(true, {}, $.fn.nag.settings, parameters), className = settings.className, selector = settings.selector, error = settings.error, namespace = settings.namespace, eventNamespace = "." + namespace, moduleNamespace = namespace + "-module", $module = $(this), $close = $module.find(selector.close), $context = $(settings.context), element = this, instance = $module.data(moduleNamespace), moduleOffset, moduleHeight, contextWidth, contextHeight, contextOffset, yOffset, yPosition, timer, module, requestAnimationFrame = window.requestAnimationFrame || window.mozRequestAnimationFrame || window.webkitRequestAnimationFrame || window.msRequestAnimationFrame || function(callback) {
            ...
            module = {
                initialize: function() {
                    ...
                    if (settings.sticky) {
                        ...
                        if (settings.position == "absolute") {
                            $context.on("scroll" + eventNamespace, module.event.scroll).on("resize" + eventNamespace, module.event.scroll);
                        } else {
                            $(window).on("scroll" + eventNamespace, module.event.scroll).on("resize" + eventNamespace, module.event.scroll);
                        }
                ...
                stick: function() {
                    ...
                    if (settings.position == "fixed") {
                        var windowScroll = $(window).prop("pageYOffset") || $(window).scrollTop(), fixedOffset = $module.hasClass(className.bottom) ? contextOffset.top + (contextHeight - moduleHeight) - windowScroll : contextOffset.top - windowScroll;
                        $module.css({
                            position: "fixed",
                            top: fixedOffset,
