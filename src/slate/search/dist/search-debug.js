/*
 * # SlateUI extend Semantic - Search
 ...
 *
 */
define("slate/search/1.0.0/search-debug", [ "$-debug" ], function(require, exports, module) {
    var $ = require("$-debug");
    $.fn.search = function(source, parameters) {
        ...
        $(this).each(function() {
            var settings = $.extend(true, {}, $.fn.search.settings, parameters), className = settings.className, selector = settings.selector, error = settings.error, namespace = settings.namespace, eventNamespace = "." + namespace, moduleNamespace = namespace + "-module", $module = $(this), $prompt = $module.find(selector.prompt), $searchButton = $module.find(selector.searchButton), $results = $module.find(selector.results), $result = $module.find(selector.result), $category = $module.find(selector.category), element = this, instance = $module.data(moduleNamespace), module;
            module = {
                initialize: function() {
                    module.verbose("Initializing module");
                    var prompt = $prompt[0], inputEvent = prompt.oninput !== undefined ? "input" : prompt.onpropertychange !== undefined ? "propertychange" : "keyup";
                    // attach events
                ...
                handleKeyboard: function(event) {
                    ...
                    $result = $module.find(selector.result), $category = $module.find(selector.category), keyCode = event.which, keys = {
                        backspace: 8,
                        enter: 13,
                        escape: 27,
                        upArrow: 38,
                        downArrow: 40
                    }, activeClass = className.active, currentIndex = $result.index($result.filter("." + activeClass)), resultSize = $result.size(), newIndex;
                    // search shortcuts
                ...
                search: {
                    ...
                    throttle: function() {
                        var searchTerm = $prompt.val(), numCharacters = searchTerm.length;
                    ...
                    query: function() {
                        var searchTerm = $prompt.val(), cachedHTML = module.search.cache.read(searchTerm);
                        if (cachedHTML) {
                    ...
                    local: function(searchTerm) {
                        var results = [], fullTextResults = [], searchFields = $.isArray(settings.searchFields) ? settings.searchFields : [ settings.searchFields ], searchRegExp = new RegExp("(?:s|^)" + searchTerm, "i"), fullTextRegExp = new RegExp(searchTerm, "i"), searchHTML;
                ...
                results: {
                    generate: function(response) {
                        ...
                        var template = settings.templates[settings.type], html = "";
                    ...
                    select: function(event) {
                        ...
                        var $result = $(this), $title = $result.find(".title"), title = $title.html();
                        if (settings.onSelect == "default" || $.proxy(settings.onSelect, this)(event) == "default") {
                            var $link = $result.find("a[href]").eq(0), href = $link.attr("href") || false, target = $link.attr("target") || false;
                            module.results.hide();
                            $prompt.val(title);
                            if (href) {
